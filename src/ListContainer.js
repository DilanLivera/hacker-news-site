...
class ListContainer extends Component {
  constructor(props) {
    ...
    this.handleArticleOpenClick = this.handleArticleOpenClick.bind(this);
    this.handleCloseArticleClick = this.handleCloseArticleClick.bind(this);
  }
  ...
  async displayStories(btn) {
    ...
    const storyIdList = await fetch(getStories.url).then(data => data.json());
  ...
  handleArticleOpenClick(evt) {
    this.setState({ showArticle: true, articleUrl: evt.target.getAttribute("dataurl") });
  }

  handleCloseArticleClick() {
    this.setState({ showArticle: false, articleUrl: null });
  }
  ...
  render() {
    ...
    if (stories && stories.length > 0) {
      views = stories.map(story => (
        <Article key={story.id} story={story} showArticle={url => this.handleArticleOpenClick(url)} />
      ));
    }
    ...
    for (let i = 0; i < numOfItems; i++) {
      paginationItems.push(
        <Pagination.Item key={i} active={i === pageActive} onClick={this.handleClick}>
          {i + 1}
        </Pagination.Item>
      );
    }
    ...
    return (
      <div className='list-container'>
        <div className='btn-container'>
          {btnList.map((btn, i) => (
            <button key={btn} onClick={this.displayStories.bind(this, btn)} className='sort-btn'>
              {btn}
            </button>
          ))}
        </div>
        {this.state.showArticle ? (
          <div>
            <object
              id='list-container-show-article'
              type='text/html'
              data={this.state.articleUrl}
              aria-label='article container'
            ></object>
            <button onClick={this.handleCloseArticleClick}>Close</button>
          </div>
        ) : (
